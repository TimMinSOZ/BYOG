...
'"FYD" holds the actual NEM grid SCADA information from Jan 2020 which is 3 imports (SCADA, Solar Rooftop, Interconnectors) whereas "125" is a scratch pad profile'
'to design a grid with and change the fuels and quantities'
...
# pythonspeed.com - Site covering memory and eficiency ### https://www.infoworld.com/article/3600993/9-fine-libraries-for-profiling-python-code.html - review of profilers
...
region = "ALL"
rfc['FYD'+"graph_fuel_order"] = ['NUC','SMR','CBR', 'CBL','GAS','SUT','SRT','WIN','HVI','IHB','EVB','EVG','HPU','HYD','BAT','GOC','NGT','ZL1','GAP','EXS','DL1']
...
rfc['FYD'+'renfuel'] = ['SUT','WIN','SRT']
rfc['FYD'+'fixfuel'] = ['CBR','CBL','GAS','NUC','SMR']
...
rfc['FYD'+'annual_ihb_sales'] = 0.057 # In millions SunWiz 2023 Home Battery Sales - Avg size = 11.5KW - Updated 15/04/23
...
for col in cols :
    ...
    # color = str(col[4])
    # color=color.lower()
    color = mcolors.CSS4_COLORS[str(col[4]).lower()]
...
while True :
    ...
    if event in (None, 'Exit', '-EXIT-','escapeexit') :
        print("[LOG] Clicked Exit!")
        start_scn_w.close()
        exit()
    
    if event == 'OK' or event == 'enter' :
...
cur_obj.execute("select prefix, region, fuel, code, data from gbact_all15 where prefix = 'FYD' and code = ''")
...
rfc[pfx+"start_date"] = start_date_obj.strftime('%Y%m%d%H%M')
rfc['FYD'+'start_pos'] = rfd_start
print('start date and time = ',rfc[pfx+"start_date"], rfd_start, rfd_end)
print("SQL BLOBs finish  ", datetime.now())

##### Load the rfg (Graph) defaults
rfg['region'] = region
rfg['demline'] = 300
rfg['legsize'] = 10
rfg['legbold'] = 'normal'
...
rfg['graph_table_bold'] = 'normal'
rfg['rf_cost_table'] = 'N'
rfg['rf_cost_table_size'] = 15
rfg['rf_cost_table_bold'] = 'bold'
rfg['fuel_cost_table'] = 'N'
rfg['fuel_cost_table_size'] = 10
rfg['fuel_cost_table_bold'] = 'normal'
rfg['rf_compare_table'] = 'N'
rfg['rf_compare_table_size'] = 10
rfg['rf_compare_table_bold'] = 'normal'
...
rfg['rf_compare_options'] = ['Capital Cost', 'Annual Cost', 'LCOE Lazards', 'LCOE CSIRO', 'Exit']
rfg['rf_table_totals'] = []
